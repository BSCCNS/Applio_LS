...
Changes from previous version:
  ...
  - CosineAnnealingWarmRestarts (T_0=50, T_mult=2)
    replaces CosineAnnealingLR — periodic lr resets help
    escape local minima and widen bonafide/spoof gap
"""
...
CFG = dict(
    features         = HUBERT_COLS,  # 769 input channels
    hidden_dim       = 512,   # increased from 256 — more capacity for 769D input
    num_layers       = 2,
    dropout          = 0.1,
    max_len          = 500,
    batch_size       = 256,
    epochs           = 300,   # extended — loss still declining at 200
    lr               = 1e-3,
    clip_grad        = 1.0,
...
class LSTMPredictor(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        Returns predicted next frames: (B, T-1, n_feat)
        We pass frames 0..T-2 as input, predict frames 1..T-1.
        Lengths are decremented by 1 for the packed sequence.
        """
        # Input: all frames except the last
        x_in       = x[:, :-1, :]                         # (B, T-1, n_feat)
        lengths_in = (lengths - 1).clamp(min=1)            # at least 1
        ...
        return self.proj(out)                              # (B, T-1, n_feat)
