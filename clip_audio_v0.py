...
NATURAL_PAUSE_MS       = 100    # replace long silences with this short pause
                                 # (sounds natural, ~1 breath pause)
RMS_THRESHOLD_FACTOR   = 0.2   # fraction of peak RMS → silence
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python clip_audio.py <audio_file> [out_folder]")
        print("\nConfig:")
        print(f"  MAX_INTERNAL_SILENCE_S = {MAX_INTERNAL_SILENCE_S}s")
        print(f"  NATURAL_PAUSE_MS       = {NATURAL_PAUSE_MS}ms")
        print(f"  RMS_THRESHOLD_FACTOR   = {RMS_THRESHOLD_FACTOR}")
        sys.exit(0)
    ...
    for i, (t0, t1) in enumerate(segments):
        print(f"  Segment {i+1}: {t0:.2f}s — {t1:.2f}s  "
              f"(duration: {t1-t0:.2f}s)")

    n = clip_audio(audio_path, out_folder)
    print(f"Output: {out_folder}/{audio_path.split('/')[-1]}")
    print(f"Segments joined: {n}")
