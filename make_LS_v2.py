...
def boiler_plate(input_data_set):
...
def make_df_projected_annotated_2d(df_anotated):
    print(f'-------- umap')
    umap2 = u.train_umap(
        df_anotated,
...
def make_plot(df_proj_anotated):
    ...
    plots.make_tagged_LS_plot(df_proj_anotated,
            ...
            s = 0.1,
            show_global=True)
    plt.savefig(f'{folder_dict["folder_plots"]}/LS_{DATA_SET}_layer_{layer}')
...
algn_paths, folder_dict = boiler_plate(DATA_SET)
silhouette_dict = {}
mi_dict = {}
